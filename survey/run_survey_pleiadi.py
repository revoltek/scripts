...
import os, sys
import string, random, glob, inspect
import logging
...
class Schedule():
    ...
    def submit(self):
        ...
        print("sbatch -p lofar %s" % (self.file_sbatch))

    def prepare_sbatch(self, mode):
    
        if mode == 'cal':
            dir_orig = dir_storage_cals+'/mss/'+self.name
            ...
            content = """#!/bin/bash
                         ...
                         mv /local/work/fdg/plots* /local/work/fdg/cal*h5 /local/work/fdg/*logger /local/work/fdg/logs %s
                         rm -r /local/work/fdg/*
...
# cals
all_cals = sorted(glob.glob(dir_storage_cals+'/mss/id*'))
logging.info('Setting up %i jobs.' % len(all_cals))

for dir_orig in all_cals:
    dir_dest = dir_orig.replace('mss','done')
