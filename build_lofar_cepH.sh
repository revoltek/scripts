...
export CC=`which gcc`
export CXX=`which g++`
export CFLAGS="-D_GLIB_USE_CXX_ABI=1 -DBOOST_NO_CXX11_SCOPED_ENUMS -march=native -O3 -fno-math-errno -ftree-vectorize"
export CXXFLAGS="-D_GLIB_USE_CXX_ABI=1 -DBOOST_NO_CXX11_SCOPED_ENUMS -march=native -O3 -fno-math-errno -ftree-vectorize"
export LDFLAGS="-lgsl -lcblas"
...
if [ ! -d $INSTALLDIR/python-casacore ]; then
    ...
    export PYTHONPATH=$INSTALLDIR/python-casacore/lib/python${PYTHON_VERSION}/site-packages/:$PYTHONPATH
...
if [ ! -d $INSTALLDIR/dysco ]; then
    ...
    cd $INSTALLDIR/dysco/build && $cmake $CMAKE_RPATH -DCMAKE_CXX_FLAGS="-D_GLIB_USE_CXX_ABI=1 -march=native -O2 -fno-math-errno -ftree-vectorize" -DCMAKE_INSTALL_PREFIX=$INSTALLDIR/dysco -DCASACORE_ROOT_DIR=$INSTALLDIR/casacore ../src
...
if [ ! -d $INSTALLDIR/idg ]; then
    ...
    cd $INSTALLDIR/idg/build && $cmake $CMAKE_RPATH -DCMAKE_INSTALL_PREFIX=$INSTALLDIR/idg -DBUILD_WITH_MKL=ON -DBUILD_LIB_CPU=ON -DBUILD_LIB_CUDA=ON -DMKL_INCLUDE_DIRS:PATH=/home/fdg/node31/opt/intel/mkl/include ../src
