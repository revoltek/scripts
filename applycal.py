...
for timestep, buf in enumerate(t.iter("TIME")):
    ...
    #print buf.getcell("TIME",0), times[timestep]
    #assert buf.getcell("TIME",0) == times[timestep]

    if (soltab_tec.time[timestep]-buf[0]["TIME"]+0.5*buf[0]["INTERVAL"])>3:
    ...
    for rownr, row in enumerate(buf):
        ant1 = row["ANTENNA1"]
        ant2 = row["ANTENNA2"]
        if wgts_tec[direction,ant1,0,timestep] == 0 or wgts_tec[direction,ant2,0,timestep] == 0 or \
           wgts_csp[direction,ant1,0,timestep] == 0 or wgts_csp[direction,ant2,0,timestep] == 0:
            flag[rownr,channel,:] = True
            continue

        g1 = sols_csp[direction,ant1,0,timestep] + sols_tec[direction,ant1,0,timestep] * 8.44797245e9 / freqs
        g1 = cos(g1) + 1j*sin(g1)
        g2 = sols_csp[direction,ant2,0,timestep] + sols_tec[direction,ant2,0,timestep] * 8.44797245e9 / freqs
        g2 = cos(g2) + 1j*sin(g2)
        for pol in range(4):
