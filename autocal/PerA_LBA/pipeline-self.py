...
#model = '/home/fdg/scripts/autocal/PerA_LBA/150228_LBA-VirgoA.model'
...
cmds=[]
for ms in mss:
    cmds.append('calibrate-stand-alone --replace-sourcedb '+ms+' /home/fdg/scripts/autocal/PerA_LBA/parset_self/bbs-corbeam.parset '+fakeskymodel+' > '+ms+'-init_corbeam.log 2>&1')
#thread_cmd(cmds)
...
for i in xrange(5):
    ...
    for ms in mss_c:
        os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_ft.py '+ms+' '+model+' > '+ms+'_ft-c'+str(i)+'.log 2>&1')
    ...
    cmds=[]
    for ms in mss_c:
        cmds.append('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-selfcal_modeldata.parset msin='+ms+' cal.parmdb='+ms+'/instrument > '+ms+'_selfcal-c'+str(i)+'.log 2>&1')
    thread_cmd(cmds)
    logging.warning('Bad runs:')
    ...
    for ms in sorted(mss_c):
       ...
       os.system('losoto.py -v '+h5parm+' /home/fdg/scripts/autocal/PerA_LBA/parset_self/losoto.parset >> '+ms+'_losoto-c'+str(i)+'.log 2>&1')
    ...
    cmds=[]
    for ms in mss_c:
        cmds.append('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-selfcor.parset msin='+ms+' cor.parmdb='+ms+'/instrument > '+ms+'_selfcor-c'+str(i)+'.log 2>&1')
    thread_cmd(cmds)
    logging.warning('Bad runs:')
...
#    # avg - SB.MS:CIRC_CORRECTED_DATA -> concat-avg.MS:DATA
    ...
    os.system('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-concatavg.parset msin="['+','.join(mss_clean)+']" msout=concat-avg.MS \
            msin.datacolumn=CIRC_CORRECTED_DATA > concatavg-c'+str(i)+'.log 2>&1')

    # clean (make a new model of virgo)
    logging.info('Clean...')
    os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_clean.py concat-avg.MS iter'+str(i)+' > clean-c'+str(i)+'.log 2>&1')
...
######################
    ...
    if i == 0 or i == 4:
        ...
        cmds = []
        for j, mss_block in enumerate(np.array_split(mss, Nblocks)):
            cmds.append('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-concat.parset msin="['+','.join(mss_block)+']" msout=block_'+str(j)+'.MS > wide_concat-b'+str(j)+'.log 2>&1')
        thread_cmd(cmds)
        logging.warning('Bad runs:')
        ...
        for j in xrange(Nblocks):
            os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_ft.py block_'+str(j)+'.MS '+model+' > wide_ft-b'+str(j)+'.log 2>&1')
        # uvsub not parallel!
        logging.info('Make widefield model - UV-Subtracting Virgo A...')
        for j in xrange(Nblocks):
            os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_uvsub.py block_'+str(j)+'.MS > wide_uvsub-b'+str(j)+'.log 2>&1')

        # clean, mask, clean
        logging.info('Make widefield model - Widefield imaging...')
        for j in xrange(Nblocks):
            os.system('/home/fdg/scripts/autocal/PerA_LBA/awimager.sh block_'+str(j)+'.MS widefield_b'+str(j)+'-iter'+str(i)+' > wide_awimager1-b'+str(j)+'.log 2>&1')
            #os.system('/home/fdg/scripts/autocal/PerA_LBA/make_mask.py -t -m img/widefield_b'+str(j)+'-iter'+str(i)+'.mask img/widefield_b'+str(j)+'-iter'+str(i)+'.restored.corr > wide_makemask-b'+str(j)+'.log 2>&1')
            #os.system('/home/fdg/scripts/autocal/PerA_LBA/awimager.sh block_'+str(j)+'.MS widefieldM_b'+str(j)+'-iter'+str(i)+' img/widefield_b'+str(j)+'-iter'+str(i)+'.mask  > wide_awimager2-b'+str(j)+'.log 2>&1')
    ...
    cmds = []
    for ms in mss_c:
        cmds.append('/home/fdg/scripts/autocal/PerA_LBA/runTAMMOenv.sh NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-beamcorrupt.parset msin='+ms+' > '+ms+'_beamcorrupt-c'+str(i)+'.log 2>&1')
    thread_cmd(cmds)
    ...
    for j, ms in enumerate(mss_c):
        ...
        cmds.append('calibrate-stand-alone --replace-sourcedb '+ms+' /home/fdg/scripts/autocal/PerA_LBA/parset_self/bbs-subfield.parset widefield_b'+str(j)+'.skymodel > '+ms+'_subbeamcorr-c'+str(i)+'.log 2>&1')
    thread_cmd(cmds)
    ...
    if i==0 or i == 4:
        ...
        logging.info('Concat...')
        os.system('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-concat.parset msin="['+','.join(mss)+']" msout=concat_all.MS > flag_concat.log 2>&1')
        logging.info('Flagging residuals...')

        os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBAiparset_self/casa_ft.py concat_all.MS '+model+' > flag_ft.log 2>&1')
        os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_uvsub.py concat_all.MS > flag_uvsub.log 2>&1')
        os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_flag.py concat_all.MS > flag_flag.log 2>&1')
    ...
    logging.info('Average...')
    os.system('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-concatavg.parset msin="['+','.join(mss_clean)+']" msout=concat.MS > concatavg-c'+str(i)+'.log 2>&1')

    # clean (make a new model of virgo)
    logging.info('Clean...')
    os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_clean.py concat.MS iter'+str(i)+' > clean-c'+str(i)+'.log 2>&1')
...
cmds = []
for j, mss_block in enumerate(np.array_split(mss, Nblocks)):
    cmds.append('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-concatavg.parset msin="['+','.join(mss_block)+']" msout=concat-avg_b'+str(j)+'.MS > final_concatavg-b'+str(j)+'.log 2>&1')
    # fast version
    #cmds.append('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-concatavg.parset msin="['+','.join(mss_block)+']" msout=concat-avg_b'+str(j)+'.MS msin.datacolumn=CIRC_CORRECTED_DATA > final_concatavg-b'+str(j)+'.log 2>&1')
thread_cmd(cmds)
# avg using the block, otherwise NDPPP crashes for too many file open
os.system('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-concatavg.parset msin=concat-avg_b*MS msout=concat-avg_all.MS avg.freqstep=1 > final_concatavg-all.log 2>&1')
# fast version
#os.system('NDPPP /home/fdg/scripts/autocal/PerA_LBA/parset_self/NDPPP-concatavg.parset msin=concat-avg_b*MS msout=concat-avg_all.MS avg.freqstep=1 msin.datacolumn=DATA > final_concatavg-all.log 2>&1')

#########################################################################################################
# group images of PerA
logging.info('Block images...')
os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_clean.py concat-avg_all.MS all > final_clean-all.log 2>&1')
#cmds = []
#for j in xrange(Nblocks):
#    cmds.append('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_clean.py concat-avg_b'+str(j)+'.MS block'+str(j)+' > final_clean-b'+str(j)+'.log 2>&1')
#thread_cmd(cmds, max_threads=3)

#########################################################################################################
# low-res image (to be done at the end -> uvsub)
logging.info('Make low-resolution image...')
os.system('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/PerA_LBA/parset_self/casa_clean_lr.py concat-avg_all.MS all > final_lrclean_all.log 2>&1')
