...
def make_mask(image_name, threshpix=5, threshisl=3, atrous_do=False, mask_name=None):
    ...
    gausmodel = image_name.replace('.restored.corr','.gausmodel')
    ...
    if mask_name == None: mask_name = image_name.replace('.restored.corr','.mask')
    ...
    pixels_mask[idx] = 1.0
    pixels_mask[0,0,2400:2600,2400:2600] = 0.0 # remove any possible fake detection over virgo
