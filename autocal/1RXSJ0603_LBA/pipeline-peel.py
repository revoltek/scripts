...
def clean(c, mss, dd, groups, avgfreq=4, avgtime=10, facet=False, skip_mask=False):
    ...
    if facet:
        imsize = size_from_facet('peel/'+dd['name']+'/models/peel_facet_g'+groups[0]+'.model.ms', dd['coord'], 3)
        niter = 5000
    else:
        imsize = size_from_facet('peel/'+dd['name']+'/models/peel_g'+groups[0]+'.model.ms', dd['coord'], 3)
        niter = 2000
...
#    check_rm('peel-model_TC*.MS')
    ...
    model = clean('initfacet', peelmss, dd, groups, facet=True)
    ...
    for m in glob.glob(facetmodel+'*'):
        s.add_casa('/home/fdg/scripts/autocal/casa_comm/casa_blank.py', \
                params={'imgs':m, 'region':dd['reg_facet'], 'inverse':True}, log='final_casablank.log')
    s.run(check=True)
