...
#    os.system('casapy --nogui --log2term --nologger -c  /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self2/casa_concat.py '+msregexp+' concat.MS > split1-c'+str(i)+'.log 2>&1')
    ...
    os.system('casapy --nogui --log2term --nologger -c  /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self2/casa_clean.py concat.MS '+imagename+'-casa > cleanA-c'+str(i)+'.log 2>&1')
    os.system('/opt/cep/WSClean/wsclean-1.7/build/wsclean -reorder -name ' + imagename + '-WSC -size 5000 5000 \
            -scale 5arcsec -weight briggs 0.0 -niter 100000 -mgain 0.75 -no-update-model-required concat.MS')
    os.system('/home/fdg/scripts/autocal/1RXSJ0603_LBA/awimager.sh concat.MS '+imagename+'-awimager > awimagerA-c'+str(i)+'.log 2>&1')
    sys.exit('clean test')
...
#            -scale 5arcsec -weight briggs 0.0 -niter 100000 -mgain 0.75 -no-update-model-required -casamask '+imagename+'.newmask concat.MS')
    ...
    for ms in mss:
        cmds.append('NDPPP /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self2/NDPPP-addcol.parset msin='+ms)
    thread_cmd(cmds, max_threads=max_threads)
