...
initc = 7 # initial tc num (useful for multiple observation of same target) - tooth10==7
...
###################################################################################################
# [PARALLEL] Apply cal sol - SB.MS:DATA -> SB.MS:CALCOR_DATA (calibrator corrected data, beam corrupted, linear)
...
for i, msg in enumerate(np.array_split(sorted(glob.glob('*.MS')), ngroups)):
    ...
    # prepare concatenated time chunks (TC) - SB_group#_TC#.MS:CALCOR_DATA -> group#_TC#.MS:DATA (cal corr data, beam corrupted, linear)
