...
for ms in glob.glob('*MS'):
        cmds.append('calibrate-stand-alone --replace-sourcedb '+ms+' /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_timesplit2/bbs_correct.parset '+fakeskymodel+' > '+ms+'_cor.log  2>&1')
thread_cmd(cmds, max_threads)
...
for i, msg in enumerate(np.array_split(sorted(glob.glob('*.MS')), ngroups)):
    ...
    for tcnum in tcnums:
        ...
        cmds.append('NDPPP /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_timesplit2/NDPPP-concat.parset msin="['+','.join(group_tc)+']"  msout=group'+str(i)+'/group'+str(i)+'_TC'+tcnum+'.MS > group'+str(i)+'/NDPPP_concat_TC'+tcnum+'.log 2>&1')
    thread_cmd(cmds, max_threads)
