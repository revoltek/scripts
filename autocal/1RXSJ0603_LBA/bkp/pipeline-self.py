...
for i in xrange(3):
    ...
    if i == 0:
        ...
        for ms in mss:
            ...
            check_rm(h5parm)
            os.system('H5parm_importer.py -v -i instrument '+h5parm+' '+ms+' > '+ms+'_losoto-c'+str(i)+'.log 2>&1')
            os.system('losoto.py -v '+h5parm+' /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self/losoto.parset >> '+ms+'_losoto-c'+str(i)+'.log 2>&1')
            os.system('H5parm_exporter.py -v -i instrument -c '+h5parm+' '+ms+' >> '+ms+'_losoto-c'+str(i)+'.log 2>&1')
            check_rm(ms+'/instrument')
...
#    os.system('casapy --nogui --log2term --nologger -c  /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self/casa_concat.py '+msregexp+' concat.MS > split1-c'+str(i)+'.log 2>&1')
    ...
    os.system('casapy --nogui --log2term --nologger -c  /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self/casa_clean.py concat.MS '+imagename+'-casa > casaclean.log 2>&1')
    ...
    os.system('/home/fdg/scripts/autocal/1RXSJ0603_LBA/awimager-new.sh concat.MS '+imagename+'-awimager-new > awimager-new.log 2>&1')
    sys.exit('clean test')
    ...
    os.system('casapy --nogui --log2term --nologger -c  /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self/casa_clean.py concat.MS '+imagename+'-masked '+imagename+'.newmask > cleanB-c'+str(i)+'.log 2>&1')
...
#            -scale 5arcsec -weight briggs 0.0 -niter 100000 -mgain 0.75 -no-update-model-required -casamask '+imagename+'.newmask concat.MS')
    ...
    for ms in mss:
            cmds.append('casapy --nogui --log2term --nologger -c /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self/casa_uvsub.py '+ms+' > '+ms+'_uvsub-c'+str(i)+'.log 2>&1')
    thread_cmd(cmds, max_threads=max_threads)
    ...
    os.system('casapy --nogui --log2term --nologger -c  /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self/casa_clean-lr.py concat.MS '+imagename+' > cleanA-lr-c'+str(i)+'.log 2>&1')
...
#            -scale 15arcsec -weight briggs 0.0 -niter 100000 -mgain 0.75 -no-update-model-required -maxuv-l 3500 concat.MS')
    ...
    os.system('casapy --nogui --log2term --nologger -c  /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self/casa_clean-lr.py concat.MS '+imagename+'-masked '+imagename+'.newmask > cleanB-lr-c'+str(i)+'.log 2>&1')
...
for ms in mss:
    cmds.append('calibrate-stand-alone --replace-sourcedb '+ms+' /home/fdg/scripts/autocal/1RXSJ0603_LBA/parset_self/bbs-subfinal.parset > '+ms+'_final-sub.log 2>&1')
thread_cmd(cmds, max_threads=max_threads)

# Copy .model[tt*] and .image[tt*] images
