...
def clean(c, mss, dd, avgfreq=4, avgtime=10, facet=False, skip_mask=False):
    ...
    if facet:
        imsize = size_from_reg('peel/'+dd['name']+'/models/peel_facet-0000-model.fits', 'regions/'+dd['reg_facet'], dd['coord'], pixscale, pad=1.2)
    else:
        imsize = size_from_reg('peel/'+dd['name']+'/models/peel_dd-0000-model.fits', 'regions/'+dd['reg'], dd['coord'], pixscale, pad=1.2)
