...
model = 'toothbrush.LBA.skymodel'
modelname = 'LBA'
#model = 'toothbrush.HBA150.skymodel' # add beam (-B 1)
#modelname = 'HBA'
number_of_sbs = 30
...
for tc in xrange(10):
    ...
    for c in np.linspace(0, 121, number_of_sbs).astype(int):
        ms = 'split/all_TC0'+str(tc)+'-c'+str(c)+'.MS'
...
for i, mss in enumerate(all_mss):
    ...
    # restored cluster number 8 in RESTORED_DATA
    logging.info('Run sagecal -> restoring...')
    for ms in mss:
        s.add('~sarod/bin/sagecal -a 2 -s sagecal.model -c sagecal.clusters -F 0 -I CORRECTED_DATA -O RESTORED_DATA -t '+str(inttime)+' -n 4 -k 8 -d '+ms+' -p '+ms+'.solutions -z ignore.clusters', log=ms+'_sagecal-restore.log', cmd_type='general', processors='max')
    s.run(check=True)
