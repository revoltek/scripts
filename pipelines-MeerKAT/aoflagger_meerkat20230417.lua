--[[
 ...
 Modified for use on MeerKAT data (specific test case: MGCLS) 2023-04-17
 Author: C. J. Riseley
]]--
...
function execute(input)
  ...
  local base_threshold = 2.1  -- lower means more sensitive detection
  ...
  local threshold_factor_step = 1.4 -- How much to increase the sensitivity each iteration?
  local frequency_resize_factor = 2.0 -- Amount of "extra" smoothing in frequency direction
  local transient_threshold_factor = 1.1 -- decreasing this value makes detection of transient RFI more aggressive
  ...
  for ipol,polarization in ipairs(inpPolarizations) do
    ...
    for i=1,iteration_count-1 do
      ...
      aoflagger.low_pass_filter(resized_data, 21, 31, 3.5, 4.5)
  ...
  aoflagger.scale_invariant_rank_operator(input, 0.02, 0.0)
