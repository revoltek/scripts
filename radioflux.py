...
def printflux(rms,region,bgr=None,wht=None,fluxerr=None,minrms=0,label=''):
    """
...
def printmean(rms,region,bgr=None,wht=None,fluxerr=None,label=''):
    for rm in rms:
...
def printspidx(rms,region,bgr=None,wht=None,fluxerr=0,nsigma=5,label=''):
    freqs = []
    fluxes = []
    errors = []
    noises = []
...
def radioflux(files,fgr,bgr=None,individual=False,action='Flux',wht=None,fluxerr=0,verbose=False):
    ...
    if individual:
        for n,reg in enumerate(fg_ir):
            ...
            r=action(rms,fg,bgr=bgr,wht=wht,fluxerr=fluxerr,label=n+1)
    else:
        r=action(rms,fg_ir,bgr=bgr,wht=wht,fluxerr=fluxerr)
...
if __name__ == "__main__":
    ...
    radioflux(args.files,args.fgr,args.bgr,args.indiv,args.action,args.weights,args.fluxerr,verbose=args.verbose)
